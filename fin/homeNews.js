if (!window.Fin) Fin = {};
if (!Fin.HomeNews) Fin.HomeNews = {};

Fin.HomeNews.Create = function (opt) {
    ...
    gD.setHeader('N, Дата поч.показу,Дата кiнця показу, Назва новини, N мiсця,Зображення, URL зображення,1');
    gD.setInitWidths('70,80,80,600,40,70,180,*');
    gD.setColAlign('center,center,center,left,center,center,left,left');
    gD.setColTypes("ro,ro,ro,ro,ro,img,ro,ro");
    gD.setColSorting('int,str,str,str,str,str,str,str');
    gD.attachHeader("#rspan,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#rspan,#rspan");


    gD.init();
    gD.enableHeaderMenu("false,true,true,true,true,true,true,false");
    iasufr.enableRowselectMode(gD);
    gD.setColumnHidden(0,true);
    gD.setColumnHidden(7,true);
    ...
    function LoadData() { gD.clearAll();
        ...
        iasufr.ajax({url:'fin.Home.cls', data: {func: "HomeNewsGet", json: JSON.stringify( {idOrg:selOrg.id, dn:dn, dk:dk}) }, success: function (data) {
           ...
           //gD.sortRows(2,"str","desc");
            ...
            if (cnt>0) { for (var i = 0; i < cnt; i++) {
                title=gD.cells2(i,6).getValue();
                $(gD.cells2(i, 3)).attr('title',title);
    ...
    function InitToolBar() {
        ...
        toolbar.addButton("add", 1, iasufr.lang.ui.add, "32/toolbar_add.png", "");
        toolbar.addButton("edit", 3,iasufr.lang.ui.edit, "32/toolbar_edit.png", "");
        toolbar.addButton("del", 4, iasufr.lang.ui.delete, "32/toolbar_delete.png", "");
        if (iasufr.pGrp(1))   toolbar.addButton("sel", 5, "Oрганiзацii з новинами", "32/application_view_detail.png", "");
        ...
        toolbar.attachEvent("onClick", function (id) {
           ...
           if (id == 'del') { if  (!idRow) { iasufr.message('Вкажiть строку !'); return; }
            ...
            if ((id == "edit")||(id == "add")) {
                 if (id == "add") idRow=0;
                 iasufr.loadForm("HomeNewsEdit", {onSave: Reload, width: 900, height: 520, idOrg:selOrg.id, idRow:idRow});
            }
            if (id == "sel")    {  iasufr.loadForm("OrgSelector", {width:1000, height:800,  HomeNews:1, onSelect: OrgSelectC } );  }
    ...
    function Del() { var idRow=GetId();
                iasufr.ajax( {url:'fin.Home.cls',
                     data:{ func: "HomeNewsDel", json: JSON.stringify( {idOrg: selOrg.id, idRow:idRow } )},
